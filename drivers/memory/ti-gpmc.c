...
static int gpmc_cs_request(ofnode node, int cs, struct resource *res)
{
	...
	faddr = cpu_to_fdt64(res->start);
	addr_base = ofnode_translate_address(node, (const fdt32_t *)&faddr);
...
static int gpmc_parse_dt(struct udevice *dev, struct ti_gpmc *gpmc)
{
	...
	ret = ofnode_read_u32(dev->node, "gpmc,num-cs",
			      &val);
	if (ret < 0) {
	...
	ret = ofnode_read_u32(dev->node, "gpmc,num-waitpins",
			      &gpmc->nr_waitpins);
	if (ret < 0) {
