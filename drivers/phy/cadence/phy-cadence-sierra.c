...
static int cdns_sierra_pll_mux_set_parent(struct clk *clk, struct clk *parent)
{
	...
	struct cdns_sierra_pll_mux_sel *data = dev_get_platdata(dev);
...
U_BOOT_DRIVER(cdns_sierra_pll_mux_clk) = {
	...
	.id			= UCLASS_CLK,
	.priv_auto_alloc_size	= sizeof(struct cdns_sierra_pll_mux),
	.ops			= &cdns_sierra_pll_mux_ops,
	.probe			= cdns_sierra_pll_mux_probe,
	.platdata_auto_alloc_size = sizeof(struct cdns_sierra_pll_mux_sel) * SIERRA_NUM_CMN_PLLC,
};
...
static int cdns_sierra_bind_link_nodes(struct  cdns_sierra_phy *sp)
{
	...
	ofnode_for_each_subnode(child, dev_ofnode(dev)) {
		...
		rc = device_bind(dev, link_drv, "link", NULL, ofnode_to_offset(child), NULL);
		if (rc) {
...
static int cdns_sierra_link_probe(struct udevice *dev)
{
	...
	rst = devm_reset_bulk_get_by_node(dev, dev->node);
	if (IS_ERR(rst)) {
		ret = PTR_ERR(rst);
		dev_err(dev, "failed to get reset\n");
		return ret;
	...
	ret = cdns_sierra_get_optional(inst, dev->node);
	if (ret) {
...
U_BOOT_DRIVER(sierra_phy_link) = {
	...
	.priv_auto_alloc_size = sizeof(struct cdns_sierra_inst),
};
